...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (measurement) {
      console.log(`Saving measurement: ${measurement.value} @ ${new Date(measurement.time).toISOString()}`);
      const { error: upsertError } = await supabase.from('glucose_measurements').upsert({
    ...
    if (graph && graph.length > 0) {
      ...
      const { error: graphError } = await supabase.from('glucose_measurements').upsert(
        historicalData, 
        { onConflict: 'patient_id, timestamp' }
      );
      if (graphError) console.warn("Error upserting historical data:", graphError);
    }

    // 5. Persist updated session (CRITICAL to avoid 429)
    const newSession = client.getSession();
    const { error: updateError } = await supabase.from('provider_sessions').upsert({
      id: 'librelinkup',
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Sync complete", value: measurement?.value }),
