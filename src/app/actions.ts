...
export async function getLatestGlucoseAction(
  ...
  try {
    ...
    const { measurement: glucose, graph: apiGraph } = await client.getGlucose(patientId);
    ...
    if (apiGraph && apiGraph.length > 0) {
      ...
      await supabase.from('glucose_measurements').upsert(formattedGraph, { onConflict: 'patient_id, timestamp' });
    }
